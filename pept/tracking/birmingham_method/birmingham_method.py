...
class BirminghamMethod(pept.base.LineDataFilter):
    ...
    def PEPT_PCA(self, lors):
        ...
        X_pcaa = np.insert(X_pcaa, 3, np.where((X_pca[:, 2] >np.quantile(X_pca[:, 2], 0.43)) & (X_pca[:, 2] < np.quantile(X_pca[:, 2], 0.57)), True, False), axis = 1)
        ...
        plt.axvline(np.quantile(X_pca[:, 2], 0.43), color='red')
        plt.axvline(np.quantile(X_pca[:, 2], 0.57), color='red')
        plt.show()
